...
BUILD="/tmp/jr4_build_$$"
OUT="$SDIR/JungleRunner_v${VER}.ipa"

echo "╔══════════════════════════════════════════╗"
echo "║  Jungle Runner v${VER} — PBR Realistic  ║"
echo "╚══════════════════════════════════════════╝"
...
SOURCES="main.m AppDelegate.m AudioEngine.m TextureGenerator.m ParticleSystem.m GameViewController.m"
...
ldid -S "$ADIR/$APP_NAME" 2>/dev/null && echo "  🔐 Signed" || echo "  ⚠️ Unsigned (sideload ok)"

# Bundle Assets (textures)
if [ -d "$SDIR/Assets" ]; then
    cp -R "$SDIR/Assets" "$ADIR/"
    echo "  📦 Assets bundled ($(du -sh "$SDIR/Assets" | cut -f1))"
fi
...
echo ""
echo "╔══════════════════════════════════════════╗"
echo "║  ✅ Jungle Runner v${VER} built!        ║"
echo "║  $(python3 -c "print(f'{os.path.getsize(\"$OUT\")/1024:.0f}')") KB — PBR Realistic Graphics    ║"
echo "╚══════════════════════════════════════════╝"
