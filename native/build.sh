...
BUILD="/tmp/jr2_build_$$"
OUT="$SDIR/JungleRunner_v${VER}.ipa"

echo "╔══════════════════════════════════════════╗"
echo "║  Jungle Runner Native v${VER}           ║"
echo "╚══════════════════════════════════════════╝"
...
for f in main.m AppDelegate.m GameViewController.m; do
    echo "  ▶ $f"
...
echo "  ▶ Linking..."
$LLD -demangle -arch arm64 -platform_version ios 14.0 16.5 -syslibroot "$SDK" -lobjc -lc++ -lc -lz -framework Foundation -framework UIKit -framework SceneKit -framework SpriteKit -framework Metal -framework QuartzCore -framework CoreGraphics -e _main -o "$BUILD/$APP_NAME" "$BUILD/obj/"*.o || { echo "❌ Link"; exit 1; }
echo "  ✅ Linked"
...
cd "$IDIR"; python3 -c "import zipfile,os;zf=zipfile.ZipFile('$OUT','w',zipfile.ZIP_DEFLATED);[zf.write(os.path.join(r,f),os.path.join(r,f)) for r,_,fs in os.walk('Payload') for f in fs]"
cd "$SDIR"
