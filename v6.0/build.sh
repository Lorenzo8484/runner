#!/usr/bin/env bash
set -euo pipefail
APP_NAME="JungleRunner" VER="${1:-6.0}"
SDIR="$(cd "$(dirname "$0")" && pwd)"
SDK="/home/alina/sdk/iPhoneOS16.5.sdk"
CLANG="/usr/bin/clang-19" LLD="/usr/bin/ld64.lld-19"
BUILD="/tmp/jr6_build_$$" OUT="$SDIR/JungleRunner_v${VER}.ipa"

echo "╔══════════════════════════════════════════╗"
echo "║  Jungle Runner v${VER} — KENNEY ASSETS ║"
echo "╚══════════════════════════════════════════╝"
...
echo "  ▶ Linking..."
$LLD -demangle -arch arm64 -platform_version ios 14.0 16.5 -syslibroot "$SDK" -lobjc -lc++ -lc -lz -framework Foundation -framework UIKit -framework SceneKit -framework SpriteKit -framework Metal -framework MetalKit -framework AVFoundation -framework QuartzCore -framework CoreGraphics -framework CoreImage -framework ImageIO -e _main -o "$BUILD/$APP_NAME" "$BUILD/obj/"*.o || { echo "❌ Link"; exit 1; }
echo "  ✅ Linked"
