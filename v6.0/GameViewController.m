...
// ─── PBR MATERIAL FROM BUNDLED TEXTURES ────────────────────
...
@implementation GameViewController {
    ...
    // Tree model names (random selection)
    NSArray *_treeNames;
    NSArray *_rockNames;
...
- (void)viewDidLoad {
    ...
    _treeNames = @[@"tree_default",@"tree_detailed",@"tree_oak",@"tree_fat",@"tree_cone",@"tree_tall",@"tree_small",@"tree_thin",@"tree_simple",@"tree_blocks",@"tree_pineDefaultA",@"tree_pineDefaultB",@"tree_pineTallA",@"tree_pineTallC",@"tree_pineRoundA",@"tree_pineRoundC",@"tree_pineSmallA",@"tree_pineSmallC"];
    _rockNames = @[@"cliff_rock",@"cliff_large_rock",@"cliff_half_rock"];
    ...
    [self setupHUD]; [self setupGestures];
...
- (SCNNode *)createTree {
    ...
    if (!tree) {
        ...
        SCNNode *tn = [SCNNode nodeWithGeometry:trunk];
        tn.position = SCNVector3Make(0,1.5,0);
        [tree addChildNode:tn];
        SCNSphere *leaf = [SCNSphere sphereWithRadius:1.5];
        leaf.materials = @[pbrMaterial(@"foliage")];
        SCNNode *ln = [SCNNode nodeWithGeometry:leaf];
        ln.position = SCNVector3Make(0,3.5,0);
...
- (SCNNode *)createRock {
    ...
    if (!rock) {
        ...
        body.materials = @[pbrMaterial(@"rock")];
        rock.geometry = body;
    }
...
- (SCNNode *)createCoin {
    ...
    rm.emission.contents = [UIColor colorWithRed:1.0 green:0.8 blue:0.1 alpha:1.0];
    ring.materials = @[rm];
...
// ═════════════════ GAME LOOP ═══════════════════════════════
- (void)renderer:(id<SCNSceneRenderer>)r updateAtTime:(NSTimeInterval)time {
    if(_gameOver)return;
    ...
    if(_jumping){_jumpVel-=20*dt;_playerY+=_jumpVel*dt;
        if(_playerY<=1.0){_playerY=1.0;_jumping=NO;_jumpVel=0;_playerModelNode.position=SCNVector3Make(0,0,0);[self switchAnimation:@"run"];
            SCNParticleSystem *imp=[ParticleSystem impactDirt];SCNNode *in=[SCNNode node];[in addParticleSystem:imp];in.position=SCNVector3Make(0,0.1,0);[_playerNode addChildNode:in];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1.0*NSEC_PER_SEC),dispatch_get_main_queue(),^{[in removeFromParentNode];});}}
    if(_sliding){_slideTimer-=dt;if(_slideTimer<=0){_sliding=NO;_playerModelNode.scale=SCNVector3Make(1,1,1);_playerY=1.0;[self switchAnimation:@"run"];}}
    ...
    if(_nextTreeZ>0&&_trees.count<35){_nextTreeZ=-14-drand48()*20;SCNNode *tree=[self createTree];
        int side=(drand48()<0.5)?-1:1;tree.position=SCNVector3Make(side*(3.2+drand48()*5.5),0,_playerNode.position.z-35);
        [_scnView.scene.rootNode addChildNode:tree];[_trees addObject:tree];}
    NSMutableArray *dt2=[NSMutableArray array];
    for(SCNNode *t in _trees){t.position=SCNVector3Make(t.position.x,t.position.y,t.position.z+_speed*dt);if(t.position.z>8)[dt2 addObject:t];}
    for(SCNNode *t in dt2){[t removeFromParentNode];[_trees removeObject:t];}
    
    // Spawn rocks
    _nextRockZ+=_speed*dt;
    if(_nextRockZ>0&&_rocks.count<8){_nextRockZ=-18-drand48()*30;int rl=(int)(drand48()*3)-1;
        SCNNode *rock=[self createRock];rock.position=SCNVector3Make(LANE_X(rl),0.25,_playerNode.position.z-35);
        [_scnView.scene.rootNode addChildNode:rock];[_rocks addObject:rock];}
    NSMutableArray *dr=[NSMutableArray array];
    for(SCNNode *r in _rocks){r.position=SCNVector3Make(r.position.x,r.position.y,r.position.z+_speed*dt);
        if(r.position.z>5){[dr addObject:r];continue;}
        float dx=fabsf(r.position.x-_playerNode.position.x),dz=fabsf(r.position.z),ht=_sliding?0.45:0.65;
        if(dx<ht&&dz<0.6&&_invincibleTimer<=0&&!_jumping&&_playerY<1.2){_lives--;_invincibleTimer=1.5;[[AudioEngine shared] playHit];
            SCNParticleSystem *hf=[ParticleSystem impactDirt];SCNNode *hn=[SCNNode node];[hn addParticleSystem:hf];hn.position=r.position;
            [_scnView.scene.rootNode addChildNode:hn];dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1.5*NSEC_PER_SEC),dispatch_get_main_queue(),^{[hn removeFromParentNode];});
            ...
            SCNAction *shake=[SCNAction sequence:@[[SCNAction moveBy:SCNVector3Make(0.15,0.1,0)duration:0.03],[SCNAction moveBy:SCNVector3Make(-0.3,-0.15,0)duration:0.03],[SCNAction moveBy:SCNVector3Make(0.15,0.05,0)duration:0.03],[SCNAction moveTo:_cameraNode.position duration:0.03]]];
            [_cameraNode runAction:shake];
            if(_lives<=0){[self doGameOver];return;}
            [r removeFromParentNode];[dr removeObject:r];[_rocks removeObject:r];}}
    for(SCNNode *r in dr){[r removeFromParentNode];[_rocks removeObject:r];}
    
    // Coins
    _nextCoinZ+=_speed*dt;
    if(_nextCoinZ>0&&_coinObjects.count<12){_nextCoinZ=-4-drand48()*10;int cl=(int)(drand48()*3)-1;
        SCNNode *coin=[self createCoin];coin.position=SCNVector3Make(LANE_X(cl),1.2,_playerNode.position.z-28);
    ...
    for(SCNNode *c in _coinObjects){c.position=SCNVector3Make(c.position.x,c.position.y,c.position.z+_speed*dt);
        ...
        if(dx<0.7&&dz<0.7){_coins++;[[AudioEngine shared] playCoin];
            SCNParticleSystem *burst=[ParticleSystem coinBurst];SCNNode *bn=[SCNNode node];[bn addParticleSystem:burst];bn.position=c.position;
            [_scnView.scene.rootNode addChildNode:bn];dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1.0*NSEC_PER_SEC),dispatch_get_main_queue(),^{[bn removeFromParentNode];});
            [c removeFromParentNode];[dc removeObject:c];[_coinObjects removeObject:c];}}
...
- (void)updFPS:(NSTimeInterval)time {_fpsCount++;if(time-_lastFps>=0.5){int fps=(int)(_fpsCount/(time-_lastFps));_fpsLabel.text=[NSString stringWithFormat:@"%d FPS",fps];_fpsLabel.fontColor=fps>50?[SKColor greenColor]:(fps>30?[SKColor yellowColor]:[SKColor redColor]);_fpsCount=0;_lastFps=time;}}

- (void)doGameOver {_gameOver=YES;[self switchAnimation:@"die"];[[AudioEngine shared] playDeath];
    SCNParticleSystem *dp=[ParticleSystem impactDirt];dp.birthRate=80;dp.particleColor=[UIColor redColor];
    SCNNode *dn=[SCNNode node];[dn addParticleSystem:dp];dn.position=SCNVector3Make(0,0.8,0);[_playerNode addChildNode:dn];
    SKLabelNode *go=[SKLabelNode labelNodeWithFontNamed:@"HelveticaNeue-Bold"];go.text=@"GAME OVER";go.fontSize=42;go.fontColor=[SKColor redColor];go.position=CGPointMake(_hudScene.size.width/2,_hudScene.size.height/2);[_hudScene addChild:go];
    SKLabelNode *rs=[SKLabelNode labelNodeWithFontNamed:@"HelveticaNeue"];rs.text=@"Tap to restart";rs.fontSize=20;rs.fontColor=[SKColor whiteColor];rs.position=CGPointMake(_hudScene.size.width/2,_hudScene.size.height/2-50);[_hudScene addChild:rs];
    UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(restartGame)];[_scnView addGestureRecognizer:tap];
}

- (void)restartGame {_lives=3;_score=0;_coins=0;_distance=0;_speed=10;_lane=0;_laneX=0;_playerY=1.0;_jumping=NO;_sliding=NO;_invincibleTimer=0;_gameOver=NO;_playerNode.position=SCNVector3Make(0,1.0,0);_playerModelNode.scale=SCNVector3Make(1,1,1);_playerModelNode.position=SCNVector3Make(0,0,0);[self switchAnimation:@"idle"];
    for(SCNNode *r in _rocks)[r removeFromParentNode];for(SCNNode *c in _coinObjects)[c removeFromParentNode];
    [_rocks removeAllObjects];[_coinObjects removeAllObjects];[_hudScene removeAllChildren];[self setupHUD];
    for(UIGestureRecognizer *g in _scnView.gestureRecognizers)if([g isKindOfClass:[UITapGestureRecognizer class]])[_scnView removeGestureRecognizer:g];
