...
- (void)viewDidLoad {
    ...
    // ─── FLOOR (ground plane beyond road) ───
    SCNFloor *floor = [SCNFloor floor];
    floor.reflectivity = 0.0;
    SCNMaterial *floorMat = [SCNMaterial material];
    floorMat.lightingModelName = SCNLightingModelPhysicallyBased;
    floorMat.diffuse.contents = genColorImage(2048, ^(int x, int y, float *c) {
        float fx = x/2048.0, fy = y/2048.0;
        float n = fbm(fx*6, fy*6, 5);
        c[0]=0.22+n*0.12; c[1]=0.35+n*0.18; c[2]=0.10+n*0.08;
    });
    floorMat.roughness.contents = genImage(2048, ^float(int x, int y) {
        return 0.75 + fbm(x/2048.0*8, y/2048.0*8, 3) * 0.2;
    });
...
- (SCNNode *)makeRoadTile:(int)index {
    ...
    // Asphalt base color with procedural detail
    int size = 1024;
    mat.diffuse.contents = genColorImage(size, ^(int x, int y, float *c) {
        float fx = x/(float)size, fy = y/(float)size;
        float n = fbm(fx*20, fy*20, 5);
        float cracks = fbm(fx*40, fy*40, 3);
        float grey = 0.28 + n * 0.10;
        if (cracks < 0.3) grey -= 0.08;
        c[0] = c[1] = c[2] = grey;
    });
    
    // Roughness — asphalt is rough
    mat.roughness.contents = genImage(size, ^float(int x, int y) {
        return 0.65 + fbm(x/(float)size*12, y/(float)size*12, 5) * 0.25;
    });
    
    mat.metalness.contents = genImage(size, ^float(int x, int y) {
        return 0.02;
    });
    
    // Normal map for road texture
    mat.normal.contents = genColorImage(size, ^(int x, int y, float *c) {
        float dx = fbm((x+2)/(float)size*16, y/(float)size*16, 4) - fbm(x/(float)size*16, y/(float)size*16, 4);
        float dy = fbm(x/(float)size*16, (y+2)/(float)size*16, 4) - fbm(x/(float)size*16, y/(float)size*16, 4);
        c[0] = 0.5+dx*2.0; c[1] = 0.5+dy*2.0; c[2] = 1.0;
    });
    
    // Lane markings — white dashed line
    if (index % 2 == 0) {
        // Generate texture with lane markings
        mat.diffuse.contents = genColorImage(size, ^(int x, int y, float *c) {
            float fx = x/(float)size, fy = y/(float)size;
            float n = fbm(fx*20, fy*20, 5);
            float grey = 0.28 + n * 0.10;
            
            // White dashes in center
            float centerDist = fabs(fx - 0.5);
            float dash = (fy > 0.25 && fy < 0.75 && centerDist < 0.02) ? 0.85 : 0;
            // Edge lines
            float edgeLine = ((fx < 0.04 || fx > 0.96) && fy > 0.1 && fy < 0.9) ? 0.7 : 0;
            
            grey = MAX(grey, MAX(dash, edgeLine));
            c[0] = c[1] = c[2] = grey;
        });
    }
...
- (SCNNode *)createTree {
    ...
    SCNMaterial *tMat = [SCNMaterial material];
    tMat.lightingModelName = SCNLightingModelPhysicallyBased;
    tMat.diffuse.contents = [TextureGenerator pbrWoodMaterial].contents;
    tMat.roughness.contents = @0.7;
    tMat.metalness.contents = @0.0;
    ...
    SCNMaterial *leafMat = [SCNMaterial material];
    leafMat.lightingModelName = SCNLightingModelPhysicallyBased;
    leafMat.diffuse.contents = genColorImage(512, ^(int x, int y, float *c) {
        float n = fbm(x/512.0*8, y/512.0*8, 5);
        c[0]=0.08+n*0.1; c[1]=0.30+n*0.25; c[2]=0.08+n*0.08;
    });
    leafMat.roughness.contents = @0.7;
    leafMat.metalness.contents = @0.0;
    leafMat.transparent.contents = [UIColor colorWithWhite:0.1 alpha:1.0]; // Subsurface faking
...
- (SCNNode *)createRock {
    ...
    // Compound rock geometry
    SCNNode *main = [SCNNode node];
    SCNSphere *body = [SCNSphere sphereWithRadius:0.5];
    SCNMaterial *rm = [SCNMaterial material];
    rm.lightingModelName = SCNLightingModelPhysicallyBased;
    rm.diffuse.contents = genColorImage(512, ^(int x, int y, float *c) {
        float n = fbm(x/512.0*10, y/512.0*10, 6);
        float g = 0.35 + n * 0.25;
        c[0]=c[1]=c[2]=g;
    });
    rm.roughness.contents = @0.65;
    rm.metalness.contents = @0.05;
    body.materials = @[rm];
    main.geometry = body;
